...
# Nome do arquivo de saída baseado na data atual
current_date = datetime.now()
output_file_name = f"Anime_list_{current_date.year}_{current_date.month:02d}.csv"
...
# Verifica se há um arquivo existente no bucket (se necessário, podemos lidar com múltiplos arquivos antigos)
existing_titles = []
for blob in bucket.list_blobs():
    # Baixar apenas o arquivo mais recente no formato esperado, se existir
    if "Anime_list" in blob.name and blob.name.endswith(".csv"):
        temp_path = f"/dbfs/tmp/{blob.name}"
        blob.download_to_filename(temp_path)
        existing_data_df = spark.read.csv(temp_path, header=True, inferSchema=True)
        os.remove(temp_path)  # Remove o arquivo local após o uso
        # Coletar todos os títulos existentes no bucket
        existing_titles += [row['TITLE_ENGLISH'] for row in existing_data_df.select('TITLE_ENGLISH').distinct().collect()]

# Elimina duplicatas na lista de títulos existentes
...
if response.status_code == 200:
    ...
    # Processa os dados em uma lista de dicionários, excluindo títulos existentes no GCS
    anime_data = []
    for entry in entries:
        media = entry['media']
        # Exclui títulos que já existem no bucket
        if media['title']['english'] not in existing_titles:
            anime_data.append({
    ...
    new_data_df = spark.createDataFrame(anime_data, schema=schema)

    # Salva o novo arquivo no GCS com o nome baseado no mês e ano
    output_path = f"gs://{bucket_name}/{output_file_name}"
    new_data_df.write.csv(output_path, header=True, mode='overwrite')

    print(f"Novo arquivo {output_file_name} enviado para o bucket {bucket_name}.")
else:
